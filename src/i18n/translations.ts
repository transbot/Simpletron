  ...
  zh: {
    ...
    examples: {
      ...
      findMax: {
        ...
        storeOriginalCount: '存储原始计数到地址01',
        readFirst: '读取第一个数字到地址10',
        loadFirst: '加载该数字到累加器',
        storeMax: '存储为当前最大值到地址00',
        loadOriginalCount: '加载原始计数',
        subtract1: '减去1（地址25存储常量1）',
        storeRemainingCount: '存储剩余计数到地址02',
        loadRemainingCount: '加载剩余计数',
        branchIfDone: '如果为0（所有数字已处理完毕），跳转到19',
        readNext: '读取下一个数字到地址10',
        loadMax: '加载当前最大值',
        subNew: '减去新数字',
        branchIfNewIsLarger: '如果结果为负（新数字更大），跳转到21更新最大值',
        writeMax: '显示最大值',
        halt: '停止',
        loadNew: '加载新数字到累加器',
        updateMax: '更新最大值',
        loadRemainingCount: '加载剩余计数',
        subtract1Again: '再减去1',
        storeNewRemainingCount: '存储新的剩余计数',
        constant1: '常量1'
      }
    }
  },
  ...
  en: {
    ...
    examples: {
      ...
      findMax: {
        ...
        storeOriginalCount: 'Store original count to address 01',
        ...
        loadOriginalCount: 'Load original count',
        subtract1: 'Subtract 1 (address 25 stores constant 1)',
        storeRemainingCount: 'Store remaining count to address 02',
        loadRemainingCount: 'Load remaining count',
        branchIfDone: 'If 0 (all numbers processed), branch to 19',
        readNext: 'Read next number to address 10',
        loadMax: 'Load current maximum',
        subNew: 'Subtract new number',
        branchIfNewIsLarger: 'If negative (new number is larger), branch to 21 to update max',
        writeMax: 'Display maximum',
        halt: 'Halt',
        loadNew: 'Load new number to accumulator',
        updateMax: 'Update maximum',
        loadRemainingCount: 'Load remaining count',
        subtract1Again: 'Subtract 1 again',
        storeNewRemainingCount: 'Store new remaining count',
